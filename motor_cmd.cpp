...
void set_motor_speed(char **cmd_strings, char* return_data) {
  ...
  int pwm = (int) (p/100.0*255);
  if(!strcmp(motor, "L")) {
    // set left motor speed
    if (pwm == 0) {
      analogWrite(L_MOTOR_PWM_PIN, 0);
      digitalWrite(L_MOTOR_DIR_PIN, LOW);    // Sets left motor in forward direction. Depends on connections
      ...
      digitalWrite(L_MOTOR_DIR_PIN, LOW);     // Sets left motor in forward direction. Depends on connections
      digitalWrite(L_MOTOR_SLEEP_PIN, HIGH);  // Keep motor ON
    } else {
    ...
    if (pwm == 0) {
      ...
      digitalWrite(R_MOTOR_DIR_PIN, HIGH);   // Sets left motor in forward direction. Depends on connections
      digitalWrite(R_MOTOR_SLEEP_PIN, LOW);  // give complement sleep to turn off motors
    } else if(pwm > 0 && pwm <= 255)  {
      analogWrite(R_MOTOR_PWM_PIN, pwm);
      digitalWrite(R_MOTOR_DIR_PIN, HIGH);   // Sets left motor in forward direction. Depends on connections
      digitalWrite(R_MOTOR_SLEEP_PIN, HIGH); // Keep motor ON
    } else {
      strcpy(return_data, "Invalid speed percentage\nPercentage must be between 0 and 100");
      return;
    }
